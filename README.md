...
We are currently planning to use an Agent-Based Model (ABM), where each vehicle is represented as an agent moving through a simplified car park.

The basic idea is that vehicles arrive, search for a parking space, park when a space becomes available, and leave after some time.
